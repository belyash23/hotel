...
const deleteUser = () => {
    form.delete(route('freeRooms.delete', {'position': userId.value}), {
        preserveScroll: true,
...
const sort = (column) => {
    ...
    axios.get(route('freeRooms.sort', {'sort': JSON.stringify(columnsSort), 'filter': JSON.stringify(filter)})).then(result => {
...
<template>
    <Head title="Free Rooms" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Free rooms count</h2>
        </template>
                ...
                <div class="overflow-hidden bg-white shadow-md sm:rounded-lg">
                    <div class="flex flex-col">
                        <div class="overflow-x-auto -my-2 sm:-mx-6 lg:-mx-8">
                          ...
                          <span class="inline-flex py-3 px-6 w-full justify-between" @click="sort('hotel_name')">
                            Hotel
                          </span>
                                                <TextInput v-model="form.filter.hotel_name" @input="filter()"/>
                                            </th>
                                            <th scope="col" class="w-3/12 text-xs font-semibold tracking-wider text-left text-white uppercase">
