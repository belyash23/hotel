...
const deleteUser = () => {
    form.delete(route('guests.delete', {'guest': userId.value}), {
        preserveScroll: true,
...
const sort = (column) => {
    ...
    axios.get(route('guests.sort', {'sort': JSON.stringify(columnsSort), 'filter': JSON.stringify(filter)})).then(result => {
        guests.splice(0, guests.length, ...result.data.guests);
    });
}
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Guests</h2>
        </template>
                        ...
                        <div class="overflow-x-auto -my-2 sm:-mx-6 lg:-mx-8">
                          ...
                          <span class="inline-flex py-3 px-6 w-full justify-between" @click="sort('email')">
                            Email
                                                ...
                                                <TextInput v-model="form.filter.email" @input="filter()"/>
                                            </th>
                                            <th scope="col" class="w-3/12 text-xs font-semibold tracking-wider text-left text-white uppercase">
                                            </th>
                                        </tr>
                                        ...
                                        <tr v-for="(user, index) in guests" :key="user.id">
                                            ...
                                            <td class="py-4 px-6 whitespace-nowrap">
                                                ...
                                                <!-- Start Edit User -->
                                                <inertia-link
                                                    :href="route('guests.edit', { guest: user.id })"
